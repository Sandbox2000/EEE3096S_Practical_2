...
int main(void)
{
  ...
  // TODO: Sanity check for GPIOA mode and suchlike
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	 /* if (((GPIOA->IDR & GPIO_IDR_1) == 0) && (htim16.Init.Period == 999)){
		  delay(1000);
		  htim16.Instance->ARR = 500-1;
	  }

	  else if (((GPIOA->IDR & GPIO_IDR_1) == 0) && (htim16.Init.Period == 499)){
		  delay(1000);
		  htim16.Instance->ARR = 1000-1;
	  }*/


	  if((HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0) && (htim16.Instance->ARR == 499))
	      {

		  	htim16.Instance->ARR = 999;
	      }
	  else if((HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0) && (htim16.Instance->ARR == 999))
		      {

			  	htim16.Instance->ARR = 499;
		      }
...
void TIM16_IRQHandler(void)
{
	...
	currentPattern = read_from_address(count);
	if (currentPattern != patterns[count]){
		currentPattern = 0x01;
	}

	GPIOB->ODR = currentPattern;
